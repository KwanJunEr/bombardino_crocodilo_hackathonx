      ...
      <div className='px-8 py-8'>
        ...
        <div className='flex flex-row justify-between items-start mb-8'>
          <div className='space-y-2'>
            <h1 className='text-4xl font-bold text-blue-900 tracking-tight'>List of My Fishing Groups</h1>
            <p className='text-blue-600 text-lg'>Show all current groups and activities</p>
          </div>
        ...
        <div className='space-y-6'>
          ...
          <div className='w-full'>
            {groups.map((group, index) => (
              <Card key={group.id} className='hover:shadow-xl transition-all duration-300 border-blue-200 bg-white/80 backdrop-blur-sm hover:bg-white/90 w-full'>
                <CardHeader className='pb-3'>
                  <div className='flex justify-between items-start mb-2'>
                    <Badge 
                      variant={group.status === 'Active' ? 'default' : 'secondary'}
                      className={group.status === 'Active' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-100 text-blue-700'}
                    >
                      {group.status}
                    </Badge>
                    <span className='text-xs text-blue-500 font-medium'>#{group.id}</span>
                  </div>
                ...
                <CardContent className='space-y-4'>
                  ...
                  <div className='pt-2 border-t border-blue-100'>
                    <span className='text-xs text-blue-500'>Created: {group.createdAt}</span>
                  </div>
                  ...
                  <div className='flex gap-2 pt-2'>
                    ...
                    <Button size='sm' className='flex-1 bg-blue-600 hover:bg-blue-700 text-white w-full'>
                      View Details
                    </Button> </Link>
